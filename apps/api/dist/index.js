...
var getAllProducts = tryCatchWrapper(
  async (req, res, next) => {
    ...
    let products = await product.find(query).skip((page - 1) * 3).limit(responsePerPage);
...
var orderModel = import_mongoose6.default.model("order", orderSchema);
...
var paymentVerification = tryCatchWrapper(
  async (req, res, next) => {
    ...
    const expectedSignature = import_crypto.default.createHmac("sha256", "Ulw083J2EWSbkEQ748AlfVQ7").update(body.toString()).digest("hex");
    ...
    if (expectedSignature === req.body.razorpay_signature) {
      ...
      if (!order)
        return res.send({ message: "fuckOff" });
      let user = req.user;
      if (user) {
        user.cart = void 0;
        await user.save();
...
app.use(
  import_express5.default.urlencoded({
    extended: true
  })
);
